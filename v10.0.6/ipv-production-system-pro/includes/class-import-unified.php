...
class IPV_Prod_Import_Unified {
    ...
    public static function handle_channel_import() {
        ...
        $max_results = absint( $_POST['max_results'] ?? 100 );

        update_option( 'ipv_youtube_channel_id', $channel_id );

        // This would call the actual bulk import logic
        // For now, show success message
        add_settings_error( 'ipv_import', 'success',
            sprintf( __( '✅ Import da canale %s avviato (%d video max).', 'ipv-production-system-pro' ), $channel_id, $max_results ),
            'success'
        );
