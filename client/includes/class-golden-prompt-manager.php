...
class IPV_Prod_Golden_Prompt_Manager {
    ...
    public static function render_settings_page() {
        ...
        // Verifica se c'è un Golden Prompt dal vendor
        $has_vendor_prompt = function_exists( 'ipv_has_golden_prompt' ) && ipv_has_golden_prompt();
        ...
        <div class="wrap">
            ...
            <?php 
            // Mostra box status vendor
            if ( class_exists( 'IPV_Pro_Golden_Prompt_Client' ) ) {
                IPV_Pro_Golden_Prompt_Client::render_vendor_status_box();
            }
            ?>

            <?php if ( $has_vendor_prompt ) : ?>
                <div style="background: #e7f3ff; border: 1px solid #b8daff; border-radius: 8px; padding: 15px; margin-bottom: 20px;">
                    <p style="margin: 0; color: #004085;">
                        <strong>ℹ️ Nota:</strong> Il Golden Prompt del vendor ha la priorità su quello locale.
                        Il prompt qui sotto verrà usato solo se il vendor non ha configurato un prompt per la tua licenza.
                    </p>
                </div>
            <?php endif; ?>

            <div style="background: #fff; border: 1px solid #ccd0d4; padding: 20px; margin: 20px 0; border-radius: 4px;">
                <h2>📊 Statistiche Prompt Locale</h2>
                <p>
                    ...
                    <?php endif; ?>
                    <?php if ( $has_vendor_prompt ) : ?>
                        | <strong>In uso:</strong> <span style="color: #856404;">⚠️ Vendor (prioritario)</span>
                    <?php else : ?>
                        | <strong>In uso:</strong> <span style="color: #00a32a;">✅ Questo prompt</span>
                    <?php endif; ?>
                </p>
            </div>
