...
class IPV_Prod_YouTube_Chapters {
    ...
    public static function get_chapters( $video_id ) {
        // v10.2.11 - API yt.lemnoslife.com è down (404), disabilitata temporaneamente
        // I capitoli vengono generati da OpenAI nella descrizione AI
        return new WP_Error( 'ipv_chapters_disabled', 'YouTube Chapters API temporaneamente disabilitata.' );
    }

    /**
     * BACKUP - Codice originale get_chapters (API down)
     */
    public static function get_chapters_backup( $video_id ) {
        if ( empty( $video_id ) ) {
