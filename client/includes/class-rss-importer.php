...
class IPV_Prod_RSS_Importer {
    ...
    public static function init() {
        // v10.0.15 - Registra il cron job RSS (allineato a class-import-unified.php)
        add_action( 'ipv_rss_check_feed', [ __CLASS__, 'process_rss_feed' ] );
    ...
    /**
     * Attiva il cron RSS (v10.0.15 - allineato con class-import-unified.php)
     */
    public static function activate_rss_cron() {
        $schedule = get_option( 'ipv_rss_schedule', 'hourly' );

        if ( ! wp_next_scheduled( 'ipv_rss_check_feed' ) ) {
            wp_schedule_event( time() + 300, $schedule, 'ipv_rss_check_feed' );
            IPV_Prod_Logger::log( 'RSS Cron: Attivato con schedule ' . $schedule );
        }
    }

    /**
     * Disattiva il cron RSS (v10.0.15 - allineato con class-import-unified.php)
     */
    public static function deactivate_rss_cron() {
        $timestamp = wp_next_scheduled( 'ipv_rss_check_feed' );
        if ( $timestamp ) {
            wp_unschedule_event( $timestamp, 'ipv_rss_check_feed' );
