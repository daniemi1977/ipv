...
class IPV_Prod_Video_List_Columns {
    ...
    protected static function render_thumbnail_column( $post_id ) {
        // Prova diverse chiavi meta per la thumbnail
        ...
        // Fallback: costruisci URL da video ID
        if ( empty( $thumbnail ) && ! empty( $video_id ) ) {
            $thumbnail = "https://img.youtube.com/vi/{$video_id}/mqdefault.jpg";
        }
        
        // Fallback: featured image
        if ( empty( $thumbnail ) && has_post_thumbnail( $post_id ) ) {
            $thumbnail = get_the_post_thumbnail_url( $post_id, 'thumbnail' );
        }

        if ( $thumbnail ) {
            $link = $video_id ? "https://www.youtube.com/watch?v={$video_id}" : get_edit_post_link( $post_id );
            printf(
                '<a href="%s" target="_blank" title="%s">
                    <img src="%s" width="80" height="45" style="border-radius:4px;object-fit:cover;">
                </a>',
                esc_url( $link ),
                $video_id ? esc_attr__( 'Open on YouTube', 'ipv-production-system-pro' ) : esc_attr__( 'Edit', 'ipv-production-system-pro' ),
                esc_url( $thumbnail )
            );
        } else {
    ...
    /**
     * Aggiunge azioni personalizzate alle righe della lista
     *
     * @param array   $actions Array di azioni
    ...
    public static function add_row_actions( $actions, $post ) {
        ...
        if ( ! empty( $video_id ) ) {
            $refresh_url = wp_nonce_url(
                admin_url( 'admin-post.php?action=ipv_refresh_single_video&post_id=' . $post->ID ),
                'ipv_refresh_single_' . $post->ID
            );
            $actions['ipv_refresh'] = sprintf(
                '<a href="%s" title="%s">🔄 %s</a>',
                esc_url( $refresh_url ),
                esc_attr__( 'Aggiorna dati da YouTube', 'ipv-production-system-pro' ),
                esc_html__( 'Refresh YT', 'ipv-production-system-pro' )
            );
        }

        // 📝 Rigenera Trascrizione
        if ( ! empty( $video_id ) ) {
            $transcript_url = wp_nonce_url(
                admin_url( 'admin-post.php?action=ipv_regenerate_transcript&post_id=' . $post->ID ),
                'ipv_regenerate_transcript_' . $post->ID
            );
            $actions['ipv_regenerate_transcript'] = sprintf(
                '<a href="%s" title="%s">📝 %s</a>',
                esc_url( $transcript_url ),
                esc_attr__( 'Rigenera trascrizione del video', 'ipv-production-system-pro' ),
                esc_html__( 'Trascrizione', 'ipv-production-system-pro' )
            );
        }

        // ✨ Rigenera Descrizione AI
        if ( ! empty( $transcript ) ) {
            $ai_desc_url = wp_nonce_url(
                admin_url( 'admin-post.php?action=ipv_regenerate_ai_desc&post_id=' . $post->ID ),
                'ipv_regenerate_ai_desc_' . $post->ID
            );
            $actions['ipv_regenerate_ai'] = sprintf(
                '<a href="%s" title="%s">✨ %s</a>',
                esc_url( $ai_desc_url ),
                esc_attr__( 'Rigenera descrizione AI', 'ipv-production-system-pro' ),
                esc_html__( 'AI Desc', 'ipv-production-system-pro' )
            );
        }

        // Download Transcript (se disponibile)
        if ( ! empty( $transcript ) ) {
            $download_url = wp_nonce_url(
                admin_url( 'admin-ajax.php?action=ipv_download_transcript&post_id=' . $post->ID ),
                'ipv_download_transcript_' . $post->ID
            );
            $actions['download_transcript'] = sprintf(
                '<a href="%s" title="%s">📥 %s</a>',
                esc_url( $download_url ),
                esc_attr__( 'Scarica trascrizione come file TXT', 'ipv-production-system-pro' ),
                esc_html__( 'Download', 'ipv-production-system-pro' )
            );
        }
