 ...
 * @version 9.1.0
 * CHANGELOG v9.1.0:
 * - Rimosso extract_youtube_id() duplicato → usa IPV_Prod_Helpers
...
class IPV_Prod_Unified_Importer {
    ...
    public static function add_submenu() {
        add_submenu_page(
            ...
            __( 'Multi-Source Importer', 'ipv-production-system-pro' ),
            '<span class="dashicons dashicons-video-alt2"></span> ' . __( 'Multi-Source', 'ipv-production-system-pro' ),
            'manage_options',
    ...
    public static function render_page() {
        ?>
        <div class="wrap">
            <h1>🎬 <?php esc_html_e( 'Multi-Source Video Importer', 'ipv-production-system-pro' ); ?></h1>
            <p class="description"><?php esc_html_e( 'Import videos from YouTube, Vimeo or Dailymotion - auto-detection!', 'ipv-production-system-pro' ); ?></p>

            <div class="card" style="max-width: 800px; margin-top: 20px;">
                <h2><?php esc_html_e( 'Import Video', 'ipv-production-system-pro' ); ?></h2>

                <div class="ipv-import-form">
                    <p>
                        <label for="ipv-video-url"><strong><?php esc_html_e( 'Video URL:', 'ipv-production-system-pro' ); ?></strong></label>
                        <input type="url" id="ipv-video-url" class="large-text" placeholder="https://youtube.com/watch?v=... or https://vimeo.com/... or https://dailymotion.com/video/...">
                    </p>

                    <p id="ipv-detected-source" style="display:none;">
                        <span class="dashicons dashicons-yes-alt" style="color: green;"></span>
                        <strong><?php esc_html_e( 'Detected:', 'ipv-production-system-pro' ); ?></strong> <span id="ipv-source-name"></span>
                        <span id="ipv-source-icon"></span>
                    </p>

                    <p>
                        <button type="button" id="ipv-detect-btn" class="button">
                            🔍 <?php esc_html_e( 'Detect Source', 'ipv-production-system-pro' ); ?>
                        </button>
                        <button type="button" id="ipv-import-btn" class="button button-primary" disabled>
                            ⬇️ <?php esc_html_e( 'Import Video', 'ipv-production-system-pro' ); ?>
                        </button>
                    </p>

                    <div id="ipv-import-progress" style="display:none;">
                        <p><strong><?php esc_html_e( 'Importing...', 'ipv-production-system-pro' ); ?></strong></p>
                        <div class="ipv-progress-bar">
                            <div class="ipv-progress-fill"></div>
                        </div>
                        <p id="ipv-progress-text"><?php esc_html_e( 'Detecting source...', 'ipv-production-system-pro' ); ?></p>
                    </div>
                ...
                <h3><?php esc_html_e( 'Supported Formats', 'ipv-production-system-pro' ); ?></h3>
                <table class="widefat striped">
                    <thead>
                        <tr>
                            <th><?php esc_html_e( 'Platform', 'ipv-production-system-pro' ); ?></th>
                            <th><?php esc_html_e( 'URL Formats', 'ipv-production-system-pro' ); ?></th>
                            <th><?php esc_html_e( 'Example', 'ipv-production-system-pro' ); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
        ...
        jQuery(document).ready(function($) {
            ...
            let videoId = null;
            const nonce = '<?php echo esc_js( wp_create_nonce( 'ipv_unified_nonce' ) ); ?>';

            $('#ipv-detect-btn').on('click', function() {
                const url = $('#ipv-video-url').val().trim();
                if (!url) {
                    alert('<?php echo esc_js( __( 'Please enter a video URL', 'ipv-production-system-pro' ) ); ?>');
                    return;
                }

                const $btn = $(this);
                $btn.prop('disabled', true).text('🔍 <?php echo esc_js( __( 'Detecting...', 'ipv-production-system-pro' ) ); ?>');
                ...
                $.ajax({
                    ...
                    error: function() {
                        alert('<?php echo esc_js( __( 'Connection error', 'ipv-production-system-pro' ) ); ?>');
                    },
                    complete: function() {
                        $btn.prop('disabled', false).text('🔍 <?php echo esc_js( __( 'Detect Source', 'ipv-production-system-pro' ) ); ?>');
                    }
                });
            });
            ...
            $('#ipv-import-btn').on('click', function() {
                if (!detectedSource || !videoId) {
                    alert('<?php echo esc_js( __( 'Please detect the source first', 'ipv-production-system-pro' ) ); ?>');
                    return;
                }
                    ...
                    success: function(response) {
                        ...
                        if (response.success) {
                            $('#ipv-import-result')
                                .removeClass('error')
                                .addClass('success')
                                .html('✅ <strong><?php echo esc_js( __( 'Video imported successfully!', 'ipv-production-system-pro' ) ); ?></strong><br>' +
                                      'Post ID: ' + response.data.post_id + '<br>' +
                                      '<a href="' + response.data.edit_url + '" class="button"><?php echo esc_js( __( 'Edit Video', 'ipv-production-system-pro' ) ); ?></a>')
                                .fadeIn();
                            ...
                            $('#ipv-import-result')
                                .removeClass('success')
                                .addClass('error')
                                .html('❌ <strong><?php echo esc_js( __( 'Error:', 'ipv-production-system-pro' ) ); ?></strong> ' + response.data)
                                .fadeIn();
                        }
                    },
                    error: function() {
                        ...
                        $('#ipv-import-result')
                            .removeClass('success')
                            .addClass('error')
                            .text('❌ <?php echo esc_js( __( 'Connection error', 'ipv-production-system-pro' ) ); ?>')
                            .fadeIn();
                    },
                    complete: function() {
    ...
    public static function ajax_detect_source() {
        ...
        if ( empty( $url ) ) {
            wp_send_json_error( __( 'Invalid URL', 'ipv-production-system-pro' ) );
        }
    ...
    public static function detect_source( $url ) {
        // Usa helper centralizzato per rilevare la fonte
        $source = IPV_Prod_Helpers::detect_video_source( $url );

        if ( ! $source ) {
            return new WP_Error( 
                'unknown_source', 
                __( 'Unknown video source. Supported: YouTube, Vimeo, Dailymotion', 'ipv-production-system-pro' ) 
            );
        }
        ...
        if ( ! $video_id ) {
            return new WP_Error( 
                'invalid_id', 
                __( 'Could not extract video ID from URL', 'ipv-production-system-pro' ) 
            );
        }
    ...
    public static function ajax_import_video() {
        ...
        if ( empty( $source ) || empty( $video_id ) ) {
            wp_send_json_error( __( 'Missing parameters', 'ipv-production-system-pro' ) );
        }
    ...
    public static function import_video( $source, $video_id ) {
        ...
        switch ( $source ) {
            ...
            case 'dailymotion':
                ...
                break;
            default:
                return new WP_Error( 'invalid_source', __( 'Invalid source', 'ipv-production-system-pro' ) );
        }
        ...
        $existing = get_posts( [
            ...
            'meta_query' => [
                [
                    'key' => IPV_Prod_Helpers::META_VIDEO_ID,
                    'value' => $video_id,
                ],
                [
                    'key' => IPV_Prod_Helpers::META_VIDEO_SOURCE,
                    'value' => $source,
                ],
            ],
            'fields' => 'ids',
        ...
        if ( ! empty( $existing ) ) {
            return new WP_Error( 
                'already_exists', 
                sprintf( __( 'Video already imported (Post ID: %d)', 'ipv-production-system-pro' ), $existing[0] ) 
            );
        }
        ...
        IPV_Prod_Helpers::log( 'Unified import successful', [
            'source' => $source,
            'video_id' => $video_id,
            'post_id' => $post_id,
        ] );
