 ...
 *
 * ====================================
 * CHANGELOG v10.9.0 (2025-12-19) - GOLDEN PROMPT COMPLETE CLIENT-SIDE CONFIGURATION
 * ====================================
 * 
 * ✨ NUOVA FUNZIONALITÀ GOLDEN PROMPT:
 * - ✅ Pagina "🤖 AI & Prompt" completamente ridisegnata
 * - ✅ 18 checkbox FLAG per personalizzare sezioni Golden Prompt
 *   → 12 sezioni descrizione (Risorse, Descrizione, Argomenti, etc.)
 *   → 2 sezioni community (Community, Supporta)
 *   → 6 link social individuali (Telegram, Facebook, Instagram, etc.)
 * - ✅ Button "✨ Genera Golden Prompt" con POST al vendor
 * - ✅ AJAX POST a vendor: /wp-json/ipv-vendor/v1/golden-prompt/compile
 * - ✅ Invia 13 dati + 18 flags al vendor per compilazione
 * - ✅ Golden Prompt SEMPRE remoto (mai salvato locale)
 * - ✅ UI moderna con gradient viola per sezione flags
 * - ✅ Checkbox con hover effects e descrizioni
 * - ✅ Status feedback in tempo reale
 * - ✅ Error handling completo (401, 400, timeout)
 * 
 * MODIFICHE TECNICHE:
 * - ✅ class-ai-prompt-settings.php: Aggiunto get_fields() con 18 flags
 * - ✅ render_fields(): Gestione checkbox boolean
 * - ✅ render_checkbox_field(): Nuovo metodo per rendering flags
 * - ✅ JavaScript: Handler completo per POST al vendor
 * - ✅ Raccolta dati: 13 campi + 18 flags → config JSON
 * - ✅ Headers: X-License-Key per autenticazione
 * - ✅ Timeout: 30 secondi per compilazione
 * 
 * WORKFLOW COMPLETO:
 * 1. Vendor attiva Golden Prompt con switch
 * 2. Cliente vede pagina "AI & Prompt"
 * 3. Cliente compila 13 dati + sceglie 18 flags
 * 4. Click "Genera Golden Prompt"
 * 5. POST al vendor con tutti i dati
 * 6. Vendor compila template + applica flags
 * 7. ✅ Golden Prompt pronto per uso AI
 * 
 * UI/UX:
 * - Card viola gradient per sezione "Personalizza Sezioni"
 * - Grid 2 colonne per sezioni descrizione
 * - Grid 3 colonne per link social
 * - Helper text e suggerimenti
 * - Loading states e feedback visivo
 * - Conferma prima dell'invio
 * 
 * COMPATIBILITÀ:
 * - ✅ 100% compatibile con vendor v2.8.0
 * - ✅ Richiede vendor con Golden Prompt attivo
 * - ✅ Backward compatible con configurazioni esistenti
 * - ✅ Default: tutte le sezioni attive
 *
 * ====================================
 * CHANGELOG v10.8.3 (2025-12-19) - VENDOR URL CORRECTED:
 * ====================================
 * - 🔧 FIX CRITICO: Vendor URL corretto da ilpuntodivistachannel.com a aiedintorni.it
 * - ✅ DEFAULT_SERVER: https://aiedintorni.it (corretto!)
 * - 📦 File modificato: includes/class-api-client.php
 *
 * CHANGELOG v10.8.2 (2025-12-19) - CRITICAL FIX - SERVER URL:
 * - 🔧 FIX CRITICO: Menu "Licenza" e "Server" erano commentati
 * - 🔧 FIX CRITICO: DEFAULT_SERVER era vuoto causando "A valid URL was not provided"
 * - ✅ Attivato menu: IPV Videos → 🔑 Licenza
 * - ✅ Attivato menu: IPV Videos → 🌐 Server
 * - ✅ DEFAULT_SERVER: https://ilpuntodivistachannel.com
 * - 📦 File modificati: includes/class-license-manager-client.php, includes/class-api-client.php
 *
 * CHANGELOG v10.8.1 (2025-12-19) - MULTI-FORMAT LICENSE SUPPORT:
 * - 🔧 FIX: Pattern validazione license key troppo restrittivo
 * - ✅ Supporto formato corto: XXXXX-XXXXX-XXXXX (3 segmenti da 5 caratteri)
 * - ✅ Supporto formato lungo: IPV-XXXXX-XXXXX-XXXXX-XXXXX (5 segmenti)
 * - 📝 Pattern HTML aggiornato: (IPV-)?[A-Za-z0-9]{4,5}(-[A-Za-z0-9]{4,5}){2,5}
 * - 🎯 Compatibile con entrambi i formati vendor
 * - 💡 Placeholder aggiornato: XXXXX-XXXXX-XXXXX
 * - 📦 File modificato: includes/class-license-manager-client.php
 *
 * CHANGELOG v10.8.0 (Previous) - Feature updates:
 * - 🔧 FIX CRITICO: SVG giganti nella pagina Gestione Licenza
 * - ✅ Aggiunto CSS con !important per forzare dimensioni SVG
 * - 📏 SVG ora hanno dimensioni corrette (w-4=16px, w-5=20px, w-6=24px)
 * - 🎨 Regole specifiche per .ipv-card, .ipv-btn svg
 * - 🔒 max-width/max-height per tutti gli SVG
 * - 📦 File modificato: admin/assets/css/modern-client.css v1.6.1
 *
 * CHANGELOG v10.5.3 (2025-12-18) - VERIFICATION UPDATE:
 * - ✅ Verified: Activation hook already loads classes correctly (NO fatal error)
 * - ✅ Verified: class-auto-installer.php loaded before use (line 941)
 * - ✅ Code structure: Production-ready and error-free
 * - 📋 Update: Version bump for consistency
 *
 * CHANGELOG v10.5.1 (2025-12-17) - CSS FIX:
 * - Fixed: CSS senza @apply (funziona con CDN Tailwind)
 * - Fixed: SVG icons ora hanno dimensioni corrette (w-5, h-5, etc.)
 * - Fixed: Tutti gli stili .ipv-* in CSS puro
 *
 * CHANGELOG v10.5.0 (2025-12-17) - GOLDEN PROMPT CLIENT:
 * 🎯 GOLDEN PROMPT CLIENT:
 * - ✨ Nuovo modulo per ricevere Golden Prompt dal vendor
 * - 🔄 Sync automatico alla attivazione licenza
 * - 🔄 Check periodico per aggiornamenti (daily cron)
 * - 🔒 Memorizzazione offuscata (non visibile all'utente)
 * - 📡 REST API endpoint per ricevere push dal vendor
 * - 🛡️ Autenticazione tramite license key
 *
 * 📦 NEW FILE:
 * - includes/class-golden-prompt-client.php
 *
 * 🔧 HELPER FUNCTIONS:
 * - ipv_get_golden_prompt() - Ottiene il prompt (uso interno)
 * - ipv_has_golden_prompt() - Verifica se esiste un prompt
 *
 * 🔐 SECURITY:
 * - Golden Prompt MAI esposto all'utente finale
 * - Offuscato in database con base64 + reverse
 * - Solo accessibile via funzioni interne del plugin
 *
...
// PHP 8.1+ Safe Helpers (v10.5.2) - Elimina deprecated warnings
require_once IPV_PROD_PLUGIN_DIR . 'includes/ipv-safe.php';
...
// AI Prompt Config - Gestisce configurazione canale per Golden Prompt (NEW v10.6.0)
require_once IPV_PROD_PLUGIN_DIR . 'includes/class-ai-prompt-config.php';
...
// Connection Diagnostic - v10.5.2+ - Tool diagnostica 401 errors
require_once IPV_PROD_PLUGIN_DIR . 'includes/class-connection-diagnostic.php';
...
// ============================================
// GOLDEN PROMPT CLIENT v10.5.0
// Riceve e memorizza il Golden Prompt dal vendor
// ============================================
if ( file_exists( IPV_PROD_PLUGIN_DIR . 'includes/class-golden-prompt-client.php' ) ) {
    require_once IPV_PROD_PLUGIN_DIR . 'includes/class-golden-prompt-client.php';
...
class IPV_Production_System_Pro {
    ...
    private function __construct() {
        ...
        IPV_Prod_AI_Prompt_Config::init(); // NEW v10.6.0
