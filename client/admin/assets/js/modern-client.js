...
(function($) {
    ...
    // ========================================
    // SVG SIZE FIX - NUCLEAR JAVASCRIPT OPTION
    // ========================================
    
    /**
     * Force SVG dimensions via JavaScript
     * This is the NUCLEAR option when CSS fails
     */
    function forceSVGSizes() {
        // Get all SVGs in the document
        const svgs = document.querySelectorAll('svg');
        
        svgs.forEach(svg => {
            // Skip if already has explicit small size
            const currentWidth = svg.style.width || svg.getAttribute('width');
            const currentHeight = svg.style.height || svg.getAttribute('height');
            
            // If width/height are in pixels and reasonable, skip
            if (currentWidth && currentWidth.includes('px')) {
                const width = parseInt(currentWidth);
                if (width > 0 && width <= 48) return; // Already OK
            }
            
            // Check parent classes for size hints
            let targetSize = '20px'; // Default
            
            const parent = svg.closest('[class*="w-"], [class*="h-"]');
            if (parent) {
                const classes = parent.className.split(' ');
                for (const cls of classes) {
                    if (cls === 'w-4' || cls === 'h-4') targetSize = '16px';
                    else if (cls === 'w-5' || cls === 'h-5') targetSize = '20px';
                    else if (cls === 'w-6' || cls === 'h-6') targetSize = '24px';
                    else if (cls === 'w-8' || cls === 'h-8') targetSize = '32px';
                }
            }
            
            // Check SVG's own classes
            const svgClasses = svg.className.baseVal || '';
            if (svgClasses.includes('w-4') || svgClasses.includes('h-4')) targetSize = '16px';
            else if (svgClasses.includes('w-5') || svgClasses.includes('h-5')) targetSize = '20px';
            else if (svgClasses.includes('w-6') || svgClasses.includes('h-6')) targetSize = '24px';
            else if (svgClasses.includes('w-8') || svgClasses.includes('h-8')) targetSize = '32px';
            
            // FORCE dimensions via inline style (highest priority!)
            svg.style.width = targetSize;
            svg.style.height = targetSize;
            svg.style.maxWidth = targetSize;
            svg.style.maxHeight = targetSize;
        ...
        console.log('IPV Pro: Fixed ' + svgs.length + ' SVG elements');
    }
    ...
    $(document).ready(function() {
        ...
        // Run again after 500ms (for dynamic content)
        setTimeout(forceSVGSizes, 500);
        
        // Run again after 2s (for late-loading content)
        setTimeout(forceSVGSizes, 2000);
    });
    
    // Run on AJAX complete (for dynamic content loaded via AJAX)
    $(document).ajaxComplete(function() {
        setTimeout(forceSVGSizes, 100);
    });
    
    // Expose globally for manual triggering
    window.ipvFixSVGs = forceSVGSizes;
