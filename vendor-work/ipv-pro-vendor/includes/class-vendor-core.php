...
class IPV_Vendor_Core {
    ...
    public function register_health_endpoint() {
        register_rest_route( 'ipv-vendor/v1', '/health', [
            'methods' => 'GET',
            'callback' => function() {
                // v1.0.22 - Rimossa chiamata a metodo inesistente
                $api_keys_configured = [
                    'youtube' => ! empty( get_option( 'ipv_vendor_youtube_api_key', '' ) ),
                    'openai' => ! empty( get_option( 'ipv_vendor_openai_api_key', '' ) ),
                    'supadata' => ! empty( get_option( 'ipv_vendor_supadata_key', '' ) )
                ];
