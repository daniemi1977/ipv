...
class IPV_Pro_Vendor_Auto_Installer {
     ...
     * v1.0.20 - PROTEZIONE: Non cancella mai ipv_licenses se contiene dati
     */
    public static function uninstall() {
        global $wpdb;

        // v1.0.20 - PROTEZIONE CRITICA: Non cancellare licenze se esistono
        $license_count = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}ipv_licenses");
        if ( $license_count > 0 ) {
            error_log( '[IPV Vendor] UNINSTALL BLOCCATO: ' . $license_count . ' licenze trovate. Tabella ipv_licenses NON cancellata.' );
            // Non cancellare la tabella licenze - contiene dati preziosi!
            $tables = array(
                // $wpdb->prefix . 'ipv_licenses', // MAI CANCELLARE SE CONTIENE DATI
                $wpdb->prefix . 'ipv_license_activations',
                $wpdb->prefix . 'ipv_api_logs',
                $wpdb->prefix . 'ipv_security_log',
                $wpdb->prefix . 'ipv_performance_stats',
            ...
            // Solo se non ci sono licenze, cancella tutto
            $tables = array(
                $wpdb->prefix . 'ipv_licenses',
                $wpdb->prefix . 'ipv_license_activations',
                $wpdb->prefix . 'ipv_api_logs',
