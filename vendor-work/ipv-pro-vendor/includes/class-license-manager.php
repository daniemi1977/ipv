...
class IPV_Vendor_License_Manager {
    ...
    public function validate($license_key, $domain) {
        ...
        // v1.0.22 - Se request_domain è vuoto, accetta (retrocompatibilità)
        // Se stored_domain è vuoto, accetta (licenza non ancora attivata su un dominio)
        // Fallisce SOLO se entrambi non sono vuoti E sono diversi
        if (!empty($stored_domain) && !empty($request_domain) && $stored_domain !== $request_domain) {
     ...
     * v1.0.16 - REGOLA: 1 dominio = 1 licenza
     * Usa INSERT ... ON DUPLICATE KEY UPDATE per evitare duplicati
     */
    public function create($data) {
        ...
        // Normalizza dominio
        $domain = $this->normalize_domain($data['domain']);
        
        // v1.0.16 - Se dominio specificato, verifica se esiste già una licenza
        if ( ! empty( $domain ) ) {
            $existing = $wpdb->get_row( $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}ipv_licenses WHERE domain = %s",
                $domain
            ));
            
            if ( $existing ) {
                // AGGIORNA licenza esistente invece di crearne una nuova
                ...
                $updated = $wpdb->update(
                    ...
                    [
                        ...
                        'credits_remaining' => $data['credits_total'], // Reset crediti
                        'email' => $data['customer_email'] ?: $existing->email, // NOTA: colonna DB è 'email'
                        'status' => 'active',
                ...
                if ( $updated !== false ) {
                    error_log( sprintf(
                        '[IPV Vendor] License UPDATED for domain %s: %s | Plan: %s | Credits: %d',
                        $domain,
                        $existing->license_key,
                        $variant_slug,
                        $data['credits_total']
                    ));
                    return $this->get_by_key( $existing->license_key );
                }
        ...
        // Nessuna licenza esistente per questo dominio - crea nuova
        ...
        $insert_data = array(
            ...
            'variant_slug' => $variant_slug,
            'credits_total' => $data['credits_total'],
            'credits_remaining' => $data['credits_total'],
            'email' => $data['customer_email'], // NOTA: colonna DB è 'email'
            'status' => 'active',
            'expires_at' => $expires_at,
        ...
        if ($inserted) {
            error_log( sprintf(
                '[IPV Vendor] License CREATED: %s | Domain: %s | Plan: %s | Credits: %d | Expires: %s',
                $license_key,
                $domain ?: '(not set)',
                $variant_slug,
                $data['credits_total'],
        ...
        error_log( '[IPV Vendor] License creation FAILED: ' . $wpdb->last_error );
        return false;
    }
    ...
    public function create_license_from_order( $order_id, $product_id ) {
        ...
        if ( empty( $credits_total ) ) {
            // v1.0.16 - Default credits based on variant - PIANI ANNUALI
            $credits_map = [
                'trial' => 10,              // Una tantum
                'starter' => 600,           // 50/mese × 12
                'professional' => 1200,     // 100/mese × 12
                'business' => 6000,         // 500/mese × 12
                'golden_prompt' => 0,       // Solo template, nessun credito
                'extra_credits_10' => 10,   // Una tantum
                'extra_credits_100' => 100, // Una tantum
            ];
            $credits_total = $credits_map[ $variant_slug ] ?? 10;
        }
