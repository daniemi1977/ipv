...
class IPV_Vendor_Credits_Manager {
    ...
    /**
     * Use credits
     * v1.0.15 - Aggiunto log nel ledger per tracciamento transazioni
     * 
     * @param int $license_id License ID
     * @param int $amount Amount to deduct
     * @param string $license_key License key (for ledger)
     * @param string $video_id Optional video ID (for reference)
     */
    public function use_credits( $license_id, $amount = 1, $license_key = '', $video_id = '' ) {
        global $wpdb;
        ...
        // Ottieni balance aggiornato per ledger
        $balance_after = $wpdb->get_var( $wpdb->prepare(
            "SELECT credits_remaining FROM {$wpdb->prefix}ipv_licenses WHERE id = %d",
            $license_id
        ));

        // v1.0.15 - Log nel ledger
        if ( ! empty( $license_key ) ) {
            $ledger_table = $wpdb->prefix . 'ipv_credit_ledger';
            
            // Verifica che la tabella esista prima di scrivere
            if ( $wpdb->get_var( "SHOW TABLES LIKE '$ledger_table'" ) === $ledger_table ) {
                $wpdb->insert( $ledger_table, [
                    'license_key' => $license_key,
                    'type' => 'transcript',
                    'amount' => -$amount, // Negativo per uso
                    'balance_after' => $balance_after,
                    'ref_type' => 'video',
                    'ref_id' => $video_id ?: null,
                    'note' => $video_id ? "Trascrizione video: $video_id" : 'Trascrizione video',
        ...
        error_log( sprintf(
            'IPV Vendor: Credits used - License ID %d, Amount: %d, Balance: %d',
            $license_id,
            $amount,
            $balance_after
        ));
