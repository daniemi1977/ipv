 ...
 * ====================================
 *
 * 🔧 FIX CRITICO TRASCRIZIONE (allineato a v9.2.2):
 * - ✅ Endpoint: /v1/transcript invece di /v1/youtube/transcript
 * - ✅ Parametro url: URL completo invece di videoId
 * - ✅ Parametro mode: aggiunto (auto/native/whisper) - MANCAVA!
 * - ✅ Parametro lang: passato solo se non vuoto/auto
 * - ✅ Aggiunto debug log per troubleshooting
 *
 * CAUSA ERRORE:
 * - ❌ PRIMA: endpoint sbagliato /v1/youtube/transcript
 * - ❌ PRIMA: mancava parametro 'mode' nella query
 * - ✅ ORA: /v1/transcript?url=...&text=true&mode=auto&lang=it
 *
 * ====================================
 * CHANGELOG v1.0.22 (2025-12-21) - FIX HEALTH + DOMAIN VALIDATION
 * ====================================
 *
 * 🔧 FIX CRITICI:
 * - ✅ /health endpoint: rimosso chiamata a metodo inesistente (HTTP 500 → 200)
 * - ✅ /license/validate: ora legge site_url dalla richiesta
 * - ✅ Validazione dominio: accetta se dominio richiesta vuoto (retrocompatibilità)
 *
 * LOGICA VALIDAZIONE DOMINIO:
 * - Se stored_domain vuoto → OK (licenza non attivata)
 * - Se request_domain vuoto → OK (retrocompatibilità)
 * - Se entrambi pieni E diversi → domain_mismatch
 *
 * ====================================
 * CHANGELOG v1.0.21 (2025-12-20) - SISTEMA BACKUP LICENZE
 * ====================================
 *
 * 💾 NUOVO SISTEMA BACKUP:
 * - Pagina admin "Backup Licenze" con export/import
 * - Export JSON (completo con attivazioni)
 * - Export CSV (per Excel)
 * - Import da file JSON con opzioni merge/update
 * - Backup automatico giornaliero
 * - Lista backup con ripristino 1-click
 * - Mantenimento ultimi 30 backup
 * - Protezione directory con .htaccess
 *
 * ====================================
 * CHANGELOG v1.0.20 (2025-12-20) - PROTEZIONE LICENZE
 * ====================================
 *
 * 🛡️ PROTEZIONE CRITICA:
 * - uninstall() NON cancella più ipv_licenses se contiene dati
 * - Log di warning se si tenta di cancellare licenze esistenti
 * - Tabella licenze preservata anche in caso di reinstallazione
 *
 * ====================================
 * CHANGELOG v1.0.19 (2025-12-20) - ARCHITETTURA SAAS: PUSH SERVER_URL
 * ====================================
 *
 * 🚀 ARCHITETTURA SAAS:
 * - ✅ Risposta validate_license include server_url e api_base
 * - ✅ Risposta activate_license include server_url e api_base
 * - ✅ Il client salva automaticamente il server_url
 * - ✅ Zero configurazione manuale per l'utente
 *
 * VANTAGGI:
 * - Nessun campo "Server URL" da compilare
 * - Diagnostica sempre verde
 * - Zero errori umani
 * - UX da SaaS vero
 *
 * ====================================
 * CHANGELOG v1.0.18 (2025-12-20) - FIX SUPADATA API
 * ====================================
 *
 * 🔥 FIX CRITICO CHIAMATA SUPADATA:
 * 
 * PRIMA (SBAGLIATO):
 * - POST /v1/transcript
 ...
 * - x-api-key: {key}
 * - query: ?videoId=...&lang=...&text=true
 *
 * RISPOSTA:
 * - Prima cercava: $body['transcript']
 * - Ora cerca: $body['content']
 *
 * ====================================
 * CHANGELOG v1.0.17 (2025-12-20) - FIX LOOP SUPADATA 401
 * ====================================
 *
 * 🔥 FIX CRITICO LOOP INFINITO:
 * - ❌ PRIMA: Su 401 faceva "continue" anche con 1 sola chiave → loop
 * - ✅ ORA: Verifica se esiste next key PRIMA di continuare
 * - ✅ Se nessuna chiave disponibile → return IMMEDIATO con errore
 * - ✅ Flag retry: false nel response per bloccare retry client
 * - ✅ Log chiaro: "No fallback key available"
 *
 * COMPORTAMENTO:
 * - 1 chiave + 401 → STOP immediato, errore definitivo
 * - 2+ chiavi + 401 → prova chiave successiva
 * - Nessun loop possibile
 *
 * ====================================
 * CHANGELOG v1.0.16 (2025-12-20) - FIX LICENZE 1 DOMINIO = 1 LICENZA
 * ====================================
 *
 * 🔧 REGOLA BUSINESS: 1 dominio = 1 licenza
 *
 * FIX ARCHITETTURA LICENZE:
 * - ✅ create() ora verifica se esiste già licenza per dominio
 * - ✅ Se dominio già registrato → UPDATE invece di INSERT
 * - ✅ License key PRESERVATA su aggiornamento
 * - ✅ Crediti resettati su rinnovo/upgrade
 * - ✅ Aggiunta colonna `domain` se mancante
 * - ✅ UNIQUE KEY su domain (dopo verifica no duplicati)
 *
 * COMPORTAMENTO:
 * - Nuovo dominio → Crea nuova licenza
 * - Dominio esistente → Aggiorna licenza esistente
 * - License key resta la stessa per dominio
 * - Comportamento deterministico e prevedibile
 *
 * INCLUSO:
 * - database-fix-v1.0.16.sql (script manuale per fix esistenti)
 *
 * ====================================
 * CHANGELOG v1.0.15 (2025-12-20) - FIX ARCHITETTURA CLIENT↔VENDOR
 * ====================================
 *
 * 🔥 FIX CRITICI ARCHITETTURA:
 *
 * 1️⃣ FIX ERRORE SUPADATA → CLIENT:
 *    - ❌ PRIMA: SupaData 401 veniva passato come 401 al client
 *    - ❌ CAUSA: Client interpretava come "license invalida"
 *    - ✅ ORA: SupaData errors → HTTP 502 (Bad Gateway)
 *    - ✅ Messaggio chiaro: "SupaData API key error"
 *
 * 2️⃣ FIX LEDGER TRANSAZIONI:
 *    - ❌ PRIMA: use_credits() non loggava nel ledger
 *    - ✅ ORA: Ogni trascrizione → riga in ipv_credit_ledger
 *    - ✅ Include: license_key, video_id, amount, balance_after
 *
 * 3️⃣ FIX CACHE INVALIDA:
 *    - ❌ PRIMA: Cache vuota/corrotta veniva restituita
 *    - ✅ ORA: Valida cache (>50 chars, non vuota)
 *    - ✅ Cache invalida viene eliminata automaticamente
 *
 * 4️⃣ FIX last_error SU RETRY:
 *    - ❌ PRIMA: last_error non settato su 401/402/429
 *    - ✅ ORA: last_error sempre settato prima di continue
 *
 * CONFIGURAZIONE RICHIESTA:
 * - SupaData API Key valida in Settings → API Keys
 *
 * ====================================
 * CHANGELOG v1.0.14 (2025-12-20) - WHITELABEL READY
 * ====================================
 *
 * 🏷️ WHITELABEL:
 * - ✅ Rimosse TUTTE le chiavi API hardcoded
 * - ✅ Chiavi SupaData configurabili SOLO da admin panel
 * - ✅ Errore chiaro se chiavi non configurate
 * - ✅ Pronto per rivendita a terzi
 *
 * CONFIGURAZIONE RICHIESTA:
 * IPV Vendor → Settings → API Keys → SupaData API Key 1/2/3
 *
 * ====================================
 * CHANGELOG v1.0.13 (2025-12-20) - RIMOZIONE ROTAZIONE CHIAVI
 * ====================================
 *
 * 🔧 SEMPLIFICAZIONE:
 * - ✅ Rimossa modalità Round Robin (inutile e problematica)
 * - ✅ Sempre Fixed mode: usa prima chiave valida, fallback se errore
 * - ✅ Chiave hardcoded SEMPRE come ultimo fallback
 * - ✅ Codice più semplice e prevedibile
 *
 * COMPORTAMENTO:
 * 1. Prova chiave configurata (se presente)
 * 2. Se 401/402/429 → prova chiave successiva
 * 3. Se tutte falliscono → usa hardcoded fallback
 *
 * ====================================
 * CHANGELOG v1.0.12 (2025-12-20) - FIX SUPADATA 401 UNAUTHORIZED
 * ====================================
 *
 * 🔥 FIX CRITICO - SUPADATA 401:
 * - ✅ Aggiunto 401 alla lista errori retryable (prova chiave successiva)
 * - ✅ Chiave hardcoded SEMPRE aggiunta come fallback finale
 * - ✅ max_attempts ora dinamico basato sul numero di chiavi disponibili
 * - ✅ Se chiave configurata è invalida, usa fallback automatico
 *
 * PROBLEMA RISOLTO:
 * - ❌ Chiave SupaData invalida causava 401 Unauthorized senza retry
 * - ❌ Chiave hardcoded non veniva mai usata se c'era una chiave configurata
 *
 * FILE MODIFICATI:
 * - includes/class-api-gateway.php (get_supadata_key, retryable errors)
 *
 * ====================================
 * CHANGELOG v1.0.11 (2025-12-20) - CRITICAL BUGFIXES
 * ====================================
 *
 * 🔥 FIX CRITICO #2 - LOGGER NON BLOCCANTE:
 * - ✅ log_api_call() ora avvolto in try/catch
 * - ✅ Errori DB non bloccano più la response al client
 * - ✅ Se il logging fallisce, la trascrizione viene comunque restituita
 * - ✅ Errori loggati ma non fatali
 *
 * 🔥 FIX #5 - PHP 8.2+ COMPATIBILITÀ:
 * - ✅ Cast (string) prima di strpos() in validate_request()
 * - ✅ Cast (string) per user_agent in log_api_call()
 * - ✅ Elimina deprecated warnings PHP 8.2+
 *
 * 🔥 FIX #6 - CREDITI SCALATI SOLO SU SUCCESSO:
 * - ✅ Crediti NON scalati se trascrizione viene da CACHE
 * - ✅ Crediti NON scalati se trascrizione è vuota/invalida
 * - ✅ get_transcript() ora restituisce array con flag 'cached'
 * - ✅ Response API include flag 'cached' per debug
 *
 * PROBLEMA RISOLTO:
 * - ❌ Logger che causava errore 500 se DB falliva
 * - ❌ Crediti scalati anche su cache hit
 * - ❌ PHP deprecated warning con strpos(null, ...)
 *
 * FILE MODIFICATI:
 * - includes/class-api-gateway.php (logger try/catch, cast string, cached flag)
 * - api/endpoints/class-gateway-endpoints.php (crediti solo su successo)
 *
 * ====================================
 * CHANGELOG v1.0.10 (2025-12-20) - FIX DATABASE COLUMN ERRORS
 * ====================================
 *
 * 🔥 FIX CRITICO - DATABASE INSERT ERRORS:
 * - ✅ Aggiunta colonna 'method' obbligatoria (NOT NULL) mancante in log_api_call()
 * - ✅ Cambiato 'resource_id' → 'video_id' (nome colonna corretto nel DB)
 * - ✅ Rimossi 'response_size' e 'attempts' che non esistono nella tabella
 * - ✅ Fix get_performance_stats() per usare solo colonne esistenti
 * - ✅ Aggiunto logging errori database per debug
 *
 * PROBLEMA RISOLTO:
 * - ❌ WordPress database error Unknown column 'response_size' in 'field list'
 * - ❌ WordPress database error Unknown column 'resource_id' in 'field list'
 * - ❌ La colonna 'method' era obbligatoria ma non veniva inserita
 *
 * FILE MODIFICATI:
 * - includes/class-api-gateway.php (fix log_api_call e get_performance_stats)
 *
 * ====================================
 * CHANGELOG v1.0.7 (2025-12-20) - FIX validate_license() METHOD MISSING
 * ====================================
 *
 * 🔥 FIX CRITICO - API 500 ERROR:
 * - ✅ Metodo validate_license() mancante in License_Manager
 * - ✅ Gli endpoint API (YouTube, Gateway, Transcript) chiamano validate_license()
 * - ✅ Aggiunto metodo validate_license() che restituisce license o WP_Error
 *
 * PROBLEMA RISOLTO:
 * - ❌ Client riceveva errore 500 "critical error on this website"
 * - ❌ Call to undefined method IPV_Vendor_License_Manager::validate_license()
 *
 * FILE MODIFICATI:
 * - includes/class-license-manager.php (aggiunto validate_license())
 *
 * ====================================
...
// Constants
define( 'IPV_VENDOR_VERSION', '1.0.22' );
...
add_action( 'plugins_loaded', function() {
    if ( class_exists( 'WooCommerce' ) ) {
        ...
        if ( is_admin() ) {
            ...
            // v1.0.21 - License Backup System
            require_once IPV_VENDOR_DIR . 'includes/class-license-backup.php';
            IPV_Vendor_License_Backup::instance()->init();
        }
    }
});
