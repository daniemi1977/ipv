...
class IPV_Vendor_Gateway_Endpoints {
    ...
    public function get_transcript( $request ) {
        ...
        // v1.0.11 - FIX #6: Crediti scalati SOLO se:
        // 1. Trascrizione valida (non vuota)
        // 2. Trascrizione NON è cached (check flag 'cached' o 'from_cache')
        $is_cached = false;
        $transcript_text = '';
        
        // Gestisci sia array che oggetto dalla risposta
        if ( is_array( $transcript ) ) {
            $is_cached = ! empty( $transcript['cached'] ) || ! empty( $transcript['from_cache'] );
            $transcript_text = isset( $transcript['text'] ) ? (string) $transcript['text'] : '';
        } elseif ( is_object( $transcript ) ) {
            $is_cached = ! empty( $transcript->cached ) || ! empty( $transcript->from_cache );
            $transcript_text = isset( $transcript->text ) ? (string) $transcript->text : '';
        ...
        // Scala crediti SOLO se NON cached e trascrizione valida
        if ( ! $is_cached && ! empty( trim( $transcript_text ) ) ) {
            // v1.0.15 - Passa license_key e video_id per log nel ledger
            $credits_manager->use_credits( $license->id, 1, $license->license_key ?? '', $video_id );
            
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( '[IPV Vendor] Credit used for license ' . $license->id . ' (fresh transcript for video ' . $video_id . ')' );
            }
        } else {
            if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
                error_log( '[IPV Vendor] Credit NOT used: cached=' . ( $is_cached ? 'yes' : 'no' ) . ', transcript_empty=' . ( empty( trim( $transcript_text ) ) ? 'yes' : 'no' ) );
            }
        }
        ...
        return rest_ensure_response([
            'success' => true,
            'transcript' => is_array( $transcript ) ? $transcript['text'] : $transcript,
            'video_id' => $video_id,
            'mode' => $mode,
            'lang' => $lang,
            'cached' => $is_cached,
