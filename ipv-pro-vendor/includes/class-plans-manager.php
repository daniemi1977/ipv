...
class IPV_Vendor_Plans_Manager {
    ...
    public function get_default_plans() {
        return [
            'trial' => [
                ...
                'description' => 'Prova gratuita per testare il servizio',
                'is_active' => true,
                'sort_order' => 1,
            ],
            'starter' => [
                    ...
                    <table class="wp-list-table widefat fixed striped" style="margin-bottom: 20px;">
                        ...
                        <tbody>
                            <?php foreach ( $ipv_products as $prod ) : 
                                ...
                                $plan_slug = get_post_meta( $prod->ID, '_ipv_plan_slug', true );
                                $credits = get_post_meta( $prod->ID, '_ipv_credits_total', true );
                            ?>
                                <tr>
    ...
    private function generate_product_description( $plan ) {
        ...
        $html .= '<p>' . esc_html( $plan['description'] ?? 'Il sistema professionale per automatizzare la produzione video.' ) . '</p>';
        
        $html .= '<h4>📊 Cosa Include:</h4>';
        $html .= '<ul>';
        $html .= '<li><strong>' . $plan['credits'] . ' video</strong> importabili al ' . $this->get_period_label( $plan['credits_period'] ) . '</li>';
        $html .= '<li><strong>' . $plan['activations'] . ' sito/i</strong> WordPress attivabili</li>';
        
        foreach ( $features as $key => $feature ) {
            if ( ! empty( $plan['features'][ $key ] ) ) {
                $html .= '<li>' . $feature['icon'] . ' ' . esc_html( $feature['name'] ) . '</li>';
            }
        }
        ...
        $html .= '<li>🖼️ Download automatico thumbnail</li>';
        $html .= '<li>📥 Import singolo e massivo</li>';
        $html .= '<li>🎨 Video Wall personalizzabile</li>';
        $html .= '<li>📊 Dashboard analytics</li>';
        $html .= '</ul>';
