...
class IPV_Vendor_Analytics_Dashboard {
    ...
    private function get_current_stats() {
        ...
        // MRR (Monthly Recurring Revenue) - TODO: implement proper pricing meta
        // For now, estimate based on active subscriptions
        $active_subs = $wpdb->get_var( "
            SELECT COUNT(*)
            FROM {$wpdb->prefix}ipv_licenses
            WHERE status = 'active'
            AND variant_slug NOT IN ('trial', 'extra_credits')
        " );
        // Estimate: avg €20/month per subscription
        $stats['mrr'] = $active_subs * 20;
        ...
        // Total Credits (remaining balance across all active licenses)
